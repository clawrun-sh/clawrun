...
export function createUpstashClient(): RedisClient {
  ...
  return {
    ...
    set: (key, value, opts) => {
      if (opts?.ex && opts?.nx) {
        return redis.set(key, value, { ex: opts.ex, nx: true }) as Promise<string | "OK" | null>;
      }
      if (opts?.ex) {
        return redis.set(key, value, { ex: opts.ex }) as Promise<string | "OK" | null>;
      }
      if (opts?.nx) {
        return redis.set(key, value, { nx: true }) as Promise<string | "OK" | null>;
      }
