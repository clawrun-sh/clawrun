...
/**
 * Scan a skills directory for `*/SKILL.md` files, parse each, and return
 * a deduplicated list of command names.
 */
...
function extractAllowedTools(frontmatter: string): string[] | null {
  ...
  // Case 1: Inline YAML array — `["Bash(foo *)", "Bash(bar:*)"]`
  if (valuePart.startsWith("[")) {
    return parseInlineYamlArray(valuePart);
  }

  // Case 2: Inline space-delimited string — `Bash(foo:*) Bash(bar:*)`
